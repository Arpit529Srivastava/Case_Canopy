...
class LegalDocumentAgent:
        ...
        self.tools = [
            Tool(
                ...
                func=lambda x: PILTool()._run(
                    ...
                    contact_number=x.split("Contact: ")[1].split("\nLanguage:")[0] if "Contact: " in x else None,
                    language=x.split("Language: ")[1] if "Language: " in x else "en"
                ),
    ...
    def classify_document(self, user_input: str, language: str = "en") -> str:
...
Your response should be based on the PRIMARY purpose of the case, not secondary aspects. If the case has elements of multiple categories, choose the one that represents the main objective and impact of the case."""
    ...
    def generate_document(self, user_input: str, user_name: str, location: str, contact_number: str) -> str:
        try:
            ...
            document_type = self.classify_document(full_input, language)
            
            # Then generate the appropriate document
            if document_type == "PIL":
                return PILTool()._run(user_issue, insights, user_name, location, contact_number, language)
            elif document_type == "RTI":
                return RTITool()._run(user_issue, insights, user_name, location, contact_number, language)
            else:
                return ComplaintTool()._run(user_issue, insights, user_name, location, contact_number, language)
